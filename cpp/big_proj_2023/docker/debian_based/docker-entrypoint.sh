...
CXX_DEVELOPER_UID=$(stat --printf="%u" /{{project_name}})
CXX_DEVELOPER_GID=$(stat --printf="%g" /{{project_name}})
...
su --login cxx-developer -c "cd /{{project_name}}; pre-commit install --install-hooks"
